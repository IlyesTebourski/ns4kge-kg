# NS Methods — TypeConstraints.md

**Titre :** Type-Constrained Representation Learning in Knowledge Graphs

| Metrique | Valeur |
|---|---|
| Methodes EVALUEES (tableaux) | 2 |
| Methodes MENTIONNEES seulement (prose) | 0 |
| Precision evalues | 100% | 
| Precision mentionnes | 100% |
| Recall evalues | 67% |
| Recall mentionnes | 0% |

## A. Methodes EVALUEES (valides vs `tables_only`)

| Methode | Trouvee ? | Via | Extrait |
|---|:---:|:---:|---|
| Local Closed-World Assumption | ✅ | name | cases, it can be beneficial to apply a local closed-world assumption that approximates the semantics of rela |
| Type-Constrained Negative Sampling | ✅ | name | Xiv:1508.02593v2 [cs.AI] 28 Aug 2015 # Type-Constrained Representation Learning in Knowledge Gr |

## B. Methodes MENTIONNEES seulement (valides vs prose)

_Aucune._

## C1. Recall EVALUES — dans un tableau mais non extrait

| Methode | Ou | Via | Extrait |
|---|---|:---:|---|
| None Sampling | table | name | Freebase-150k</strong></td> <td>None</td> <td>0.327</td> <td |

## C2. Recall MENTIONNES — en prose seulement mais non extrait

| Methode | Ou | Via | Extrait |
|---|---|:---:|---|
| Bernoulli Negative Sampling | prose | name | The model is trained by minimizing the Bernoulli cost-function $$ \mathcal{L}_{mwNN} = |